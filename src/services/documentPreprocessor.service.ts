...
import { AI_MODEL_NAME, AI_MODEL_DOWNLOAD_URL } from '../config.js';
...
export class DocumentPreprocessorService {
    ...
    async extractDocumentFromPhotoWithImgProcessing(photo: Buffer, imageWidth: number, imageHeight: number): Promise<Buffer> {
    ...
    private async downloadloadAIModel() {
    ...
    async getAIModel(): Promise<ort.InferenceSession> {
        if(this.aiModel == null) {
            await this.downloadloadAIModel();
    ...
    private async saveImage(mat: cv.Mat, fileName: string, width: number, height: number, channels: number) {
    ...
    private async padImageToMakeSquare(photo: Buffer, width: number, height: number): Promise<cv.Mat> {
        ...
        //    be able to infere the corner points
        ...
        // I dont know if it is any better to use cv.BORDER_CONSTANT
        cv.copyMakeBorder(mat, mat, top+paddingOffset, bottom+paddingOffset, 
            left+paddingOffset, right+paddingOffset, cv.BORDER_REPLICATE);
    ...
    /**
     * Returns coordinate of center of polygon with largest area inside image
     * 
    ...
    private getCentroidOfMaxContour(mat: cv.Mat, threshold: number = 0.3): cv.Point {
        ...
        mat.data32F.set(mat.data32F.map((val) => {
    ...
    private cropAndFixPerspective(src: cv.Mat, dst: cv.Mat, corners: cv.Point[], 
        resultWidth: number, resultHeight: number): void {
    ...
    async extractDocumentFromPhotoWithAI(photo: Buffer, imageWidth: number, imageHeight: number): Promise<Buffer | null> {
        ...
        */
        ...
        for (let corner = 0; corner < numberOfCorners; corner++) {
            mat = cv.matFromArray(aiModelOutputSize, aiModelOutputSize, cv.CV_32FC1, 
                heatmap.subarray(corner*aiModelOutputSize*aiModelOutputSize, (corner+1)*aiModelOutputSize*aiModelOutputSize));
        ...
        const buffer = await sharp(mat.data, {
            raw: {
                width: imageWidth,
                height: imageHeight,
                channels: 3
